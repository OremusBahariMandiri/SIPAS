...
@section('content')
    ...
    @php
        $activeTab = request('tab', $isAdmin ? 'history' : 'inbox');
    ...
    <div class="idx-tab-nav" style="margin-bottom:1.25rem;">
        ...
        <a href="{{ route('data.approval.index', ['tab' => 'history', 'per_page' => $perPageNow]) }}"
            ...
            {{ $isAdmin ? 'All Approval History' : 'History' }}
        </a>
    </div>
                    ...
                    @foreach ($kepadas as $k)
                        <div class="idx-mob-card">
                            ...
                            <div class="idx-mc-footer">
                                ...
                                <div class="idx-actions">
                                    <a href="{{ route('data.approval.review', $k) }}" class="idx-btn-action idx-btn-view"
                                        title="Review">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                </div>
    ...
    @if ($activeTab === 'history')
        ...
        <div class="idx-card">
            ...
            <div class="idx-card-header">
                <span class="idx-card-title">
                    {{ $isAdmin ? 'All Approval History' : 'Approval History' }}
                ...
                <div style="display:flex;align-items:center;gap:.5rem;flex-wrap:wrap;">
                    <button type="button" id="idxBtnOpenFilter"
                        class="idx-btn-filter {{ $hasFilter ? 'has-filter' : '' }}" title="Filter data">
                    ...
                    <div class="idx-fm-body">
                        ...
                        <div class="idx-fm-group">
                            ...
                            <input type="text" name="search" id="fm_search" class="idx-fm-input"
                                value="{{ $fSearch }}" placeholder="Search subject or letter number…"
                                autocomplete="off">
                        </div>
                        ...
                        <div class="idx-fm-group">
                            ...
                            <input type="text" name="dok_type" id="fm_dok_type" class="idx-fm-input"
                                value="{{ $fDokType }}" placeholder="Search document type…" autocomplete="off">
                        </div>

                    </div>

                    <div class="idx-fm-footer">
                        <a href="{{ route('data.approval.index', ['tab' => 'history']) }}" class="idx-fm-btn-reset">
        ...
        <script>
            (function() {
                ...
                window.idxConfirmDelete = function(id, no) {
                    const modal = document.getElementById('idxModalDel');
                    if (!modal) return;
