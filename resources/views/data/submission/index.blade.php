    ...
    <div class="idx-card">
        ...
        <div class="idx-card-header">
            ...
            <div style="display:flex;align-items:center;gap:.5rem;flex-wrap:wrap;">
                ...
                <button type="button" id="idxBtnOpenFilter" class="idx-btn-filter {{ $hasFilter ? 'has-filter' : '' }}"
                    title="Filter data">
                ...
                <select class="idx-filter-select" id="idxPerPage" title="Rows per page"
                    onchange="idxChangePerPage(this.value)">
            ...
            <table class="idx-tbl" style="width:100%;">
                <thead>
                    <tr>
                        ...
                        <th>To</th>
                        <th>CC</th>
                        <th class="idx-sortable" data-col="tanggal_surat" style="width:110px;">
                ...
                <tbody>
                    @forelse($items as $item)
                        <tr>
                            ...
                            @if ($isAdmin)
                                <td class="idx-td-muted" style="font-size:.8rem;">
                                    ...
                                    @if ($item->user->jabatan ?? null)
                                        <br><span style="font-size:.72rem;">{{ $item->user->jabatan }}</span>
                                    @endif
                                </td>
                            @endif
                            ...
                            {{-- Kolom TO --}}
                            <td class="idx-td-muted" style="font-size:.8rem;">
                                {{ $item->kepada->nama_karyawan ?? '-' }}
                                @if ($item->kepada->jabatan ?? null)
                                    <br><span style="font-size:.72rem;">{{ $item->kepada->jabatan }}</span>
                                @endif
                            </td>

                            {{-- Kolom CC --}}
                            <td class="idx-td-muted" style="font-size:.78rem;">
                                @if ($item->terusans->isEmpty())
                                    <span style="color:var(--muted);">—</span>
                                @else
                                    {{-- Tampilkan CC pertama --}}
                                    <span>{{ $item->terusans->first()->user->nama_karyawan ?? '-' }}</span>
                                    @if ($item->terusans->first()->user->jabatan ?? null)
                                        <br><span
                                            style="font-size:.72rem;">{{ $item->terusans->first()->user->jabatan }}</span>
                                    @endif

                                    {{-- Jika lebih dari 1, tampilkan "and X more" --}}
                                    @if ($item->terusans->count() > 1)
                                        <br>
                                        <span style="font-size:.72rem;color:var(--muted);font-style:italic;">
                                            and {{ $item->terusans->count() - 1 }} more…
                                        </span>
                                    @endif
                                @endif
                            </td>
                            ...
                            <td class="idx-td-right">
                                <div class="idx-actions">
                                    ...
                                    @if ($item->isEditable() || $isAdmin)
                                        <button type="button" class="idx-btn-action idx-btn-del" title="Delete"
            ...
            @forelse($items as $item)
                <div class="idx-mob-card">
                    ...
                    <div class="idx-mc-meta">
                        ...
                        <span class="idx-mc-meta-item">
                            <i class="bi bi-building"></i> {{ $item->perusahaan->nama ?? '-' }}
                        </span>
                        <span class="idx-mc-meta-item">
                        ...
                        <span class="idx-mc-meta-item">
                            ...
                            To: {{ $item->kepada->nrk ?? '-' }}
                            {{ $item->kepada->nama_karyawan ? '— ' . $item->kepada->nama_karyawan : '' }}
                        </span>
                        @if ($item->terusans->isNotEmpty())
                            <span class="idx-mc-meta-item">
                                <i class="bi bi-people"></i>
                                CC:
                                @foreach ($item->terusans as $cc)
                                    {{ $cc->user->nrk ?? '-' }}@unless ($loop->last)
                                    ,
                                @endunless
                            @endforeach
                        </span>
                ...
                <div class="idx-mc-footer">
                    ...
                    <div class="idx-actions">
                        <a href="{{ route('data.submission.show', $item) }}" class="idx-btn-action idx-btn-view"
                            title="View">
                            <i class="bi bi-eye"></i>
                        </a>
                        @if ($item->isEditable())
                            <a href="{{ route('data.submission.edit', $item) }}" class="idx-btn-action idx-btn-edit"
                                title="Edit">
                                <i class="bi bi-pencil"></i>
                            </a>
                        @endif
            ...
            <div class="idx-fm-body">
                ...
                <div class="idx-fm-group">
                    ...
                    <div class="idx-fm-date-row">
                        <input type="date" name="date_from" id="fm_date_from" class="idx-fm-input"
                            value="{{ $fDateFrom }}" placeholder="Dari" title="Dari tanggal">
                        <span class="idx-fm-date-sep">→</span>
                        <input type="date" name="date_to" id="fm_date_to" class="idx-fm-input"
                            value="{{ $fDateTo }}" placeholder="Sampai" title="Sampai tanggal">
                    </div>
                </div>
                ...
                <div class="idx-fm-group">
                    ...
                    <input type="text" name="search" id="fm_search" class="idx-fm-input"
                        value="{{ $fSearch }}" placeholder="Cari perihal atau nomor surat…" autocomplete="off">
                </div>
                ...
                <div class="idx-fm-group">
                    ...
                    <input type="text" name="dok_type" id="fm_dok_type" class="idx-fm-input"
                        value="{{ $fDokType }}" placeholder="Cari jenis dokumen…" autocomplete="off">
                </div>
...
<script>
    (function() {
        ...
        backdrop?.addEventListener('click', e => {
            if (e.target === backdrop) idxFmClose();
        });
        ...
        function initSelect2() {
            $('.idx-fm-select2-single').each(function() {
                $(this).select2({
                    ...
                    language: {
                        noResults: () =>
                            '<span style="padding:.5rem .85rem;display:block;font-size:.82rem;color:var(--muted);">Tidak ada hasil</span>',
                    },
                    escapeMarkup: m => m,
                });
            });
        ...
        document.querySelectorAll('.idx-sortable').forEach(th => {
            th.addEventListener('click', function() {
                ...
                const url = new URL(window.location.href);
                const newDir = (url.searchParams.get('sort') === col && url.searchParams.get(
                        'dir') === 'asc') ?
                    'desc' : 'asc';
